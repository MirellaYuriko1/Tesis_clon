# ml/oe6_tag.py
# ------------------------------------------------------------
# OE6 - Trastorno de Ansiedad Generalizada (TAG) — Dim6
# Principal: UPPERBOUND (usa SOLO los ítems de Dim6)
# Opcional : LOSO (excluye Dim6 y usa Dim1..Dim5 + edad + género)
# Salidas: reports/oe6_tag/{tables,figs,figs_ml,tables_ml}
# ------------------------------------------------------------
...
DIM_KEY = "Dim6"  # TAG
OUTDIR = Path(__file__).parent / "reports" / "oe6_tag"
RUN_LOSO = False  # True si quieres también la variante LOSO

# ======= Textos para títulos y leyendas =======
UI = {
    "pred_title": "Ansiedad generalizada (TAG): predicho vs. esperado",
    ...
    "cm_title":        "Matriz de confusión — TAG (Random Forest)",
    "cm_title_norm":   "Matriz de confusión (porcentaje por fila) — TAG",
    "color_casos":     "Casos",
    "color_pct":       "% de casos",
    "x_pred":          "Predicho",
    "y_exp":           "Esperado",

    "roc_title": "Curvas ROC por nivel — TAG",
    "roc_x":     "Falsos positivos (FPR)",
    "roc_y":     "Verdaderos positivos (TPR)",

    "pr_title":  "Curvas Precisión–Recall por nivel — TAG",
    "pr_x":      "Recall",
    "pr_y":      "Precisión",

    "cal_title": 'Calibración de probabilidades — nivel "Muy alto" (TAG)',
    "cal_leg1":  "Observado",
    "cal_leg2":  "Perfectamente calibrado",
    "cal_x":     "Probabilidad predicha",
    "cal_y":     "Fracción observada",

    "cv_title":  "Estabilidad en validación cruzada (k=5) — F1_macro (TAG)",
    "cv_y":      "F1_macro",

    "thr_title": 'Ajuste de umbral — nivel "Muy alto" (TAG)',
    "thr_x":     "Umbral de decisión",
    "thr_y":     "Valor",

    "imp_items_title": "Aporte de cada ítem (Dim6) — TAG",
    "imp_items_x":     "Importancia (permutación)",
    "imp_items_y":     "Ítems de la subescala",

    "learn_title": "Curva de aprendizaje — Random Forest (F1_macro) — TAG",
...
def cargar_dataset():
    ...
    for _, row in df.iterrows():
        ...
        nombre_sub = DIM_NOMBRES[DIM_KEY]  # "TrastornoAnsiedadGeneralizada"
