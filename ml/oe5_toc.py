# ml/oe5_toc.py
# ------------------------------------------------------------
# OE5 - Trastorno obsesivo-compulsivo (TOC) — Dim5
# Principal: UPPERBOUND (usa SOLO los ítems de Dim5)
# Opcional : LOSO (excluye Dim5 y usa Dim1,2,3,4,6 + edad + género)
# Salidas: reports/oe5_toc/{tables,figs,figs_ml,tables_ml}
# ------------------------------------------------------------
...
DIM_KEY = "Dim5"  # TOC
OUTDIR = Path(__file__).parent / "reports" / "oe5_toc"
RUN_LOSO = False   # True si quieres también la variante LOSO

# ======= Textos para títulos y leyendas =======
UI = {
    "pred_title": "TOC (Trastorno obsesivo-compulsivo): predicho vs. esperado",
    ...
    "cm_title":        "Matriz de confusión — TOC (Random Forest)",
    "cm_title_norm":   "Matriz de confusión (porcentaje por fila) — TOC",
    "color_casos":     "Casos",
    "color_pct":       "% de casos",
    "x_pred":          "Predicho",
    "y_exp":           "Esperado",

    "roc_title": "Curvas ROC por nivel — TOC",
    "roc_x":     "Falsos positivos (FPR)",
    "roc_y":     "Verdaderos positivos (TPR)",

    "pr_title":  "Curvas Precisión–Recall por nivel — TOC",
    "pr_x":      "Recall",
    "pr_y":      "Precisión",

    "cal_title": 'Calibración de probabilidades — nivel "Muy alto" (TOC)',
    "cal_leg1":  "Observado",
    "cal_leg2":  "Perfectamente calibrado",
    "cal_x":     "Probabilidad predicha",
    "cal_y":     "Fracción observada",

    "cv_title":  "Estabilidad en validación cruzada (k=5) — F1_macro (TOC)",
    "cv_y":      "F1_macro",

    "thr_title": 'Ajuste de umbral — nivel "Muy alto" (TOC)',
    "thr_x":     "Umbral de decisión",
    "thr_y":     "Valor",

    "imp_items_title": "Aporte de cada ítem (Dim5) — TOC",
    "imp_items_x":     "Importancia (permutación)",
    "imp_items_y":     "Ítems de la subescala",

    "learn_title": "Curva de aprendizaje — Random Forest (F1_macro) — TOC",
...
def cargar_dataset():
    ...
    for _, row in df.iterrows():
        ...
        nombre_sub = DIM_NOMBRES[DIM_KEY]  # "TrastornoObsesivoCompulsivo"
